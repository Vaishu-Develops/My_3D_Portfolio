...
export default function Navbar() {
  ...
  const handleNavClick = (e, href) => {
    ...
    if (!isHome && href.startsWith('#')) {
      ...
      // Allow smooth scrolling to happen naturally, just close mobile menu
      setIsMobileMenuOpen(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      console.log("[Navbar Layout] Found pills:", pills.length);
      
      pills.forEach((pill, index) => {
        const circle = pill.querySelector('.hover-circle');
        if (!circle) {
          console.warn(`[Navbar Layout] Missing circle for pill at index ${index}`);
          return;
        }
        ...
        console.log(`[Navbar Layout] Registered timeline for pill index ${index}`);
      });
    };
