...
export default function VaporizeTextCycle({
  ...
  const fontConfig = useMemo(() => {
    ...
    // User wants sharper character-by-character effect, so we reduce spread influence
...
const updateParticles = (
  ...
  particles.forEach(particle => {
    // Sharp edge: direct comparison with vaporizeX
    ...
    if (shouldVaporize) {
      if (particle.speed === 0) {
        // Initialize particle motion - slightly upward and away
        particle.angle = (Math.random() * 0.4 - 0.2) - Math.PI / 2; 
        particle.speed = (Math.random() * 0.5 + 0.5) * MULTIPLIED_VAPORIZE_SPREAD;
      ...
      if (particle.shouldFadeQuickly) {
        ...
        const rnd = MULTIPLIED_VAPORIZE_SPREAD * 3;
        particle.velocityX = (particle.velocityX + (Math.random() - 0.5) * rnd + dx * 0.002) * damping;
        particle.velocityY = (particle.velocityY + (Math.random() - 0.5) * rnd + dy * 0.002) * damping;
        const maxV = MULTIPLIED_VAPORIZE_SPREAD * 2;
        const curV = Math.sqrt(particle.velocityX ** 2 + particle.velocityY ** 2);
        if (curV > maxV) {
          const s = maxV / curV;
          particle.velocityX *= s; particle.velocityY *= s;
        }
...
function transformValue(input: number, inputRange: number[], outputRange: number[], clamp = false): number {
  const [inMin, inMax] = inputRange; const [outMin, outMax] = outputRange;
  const progress = (input - inMin) / (inMax - inMin);
  let res = outMin + progress * (outMax - outMin);
  if (clamp) {
