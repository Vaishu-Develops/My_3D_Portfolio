...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  const [cardTexture, lanyardTexture] = useMemo(() => {
    ...
    ctx.fillText('VAISHNAVI', 256, 550);

    // Title
    ctx.fillStyle = '#94a3b8'; // Slate 400
    ctx.font = '500 35px Inter, sans-serif';
    ctx.fillText('FULL STACK ENGINEER', 256, 630);
    ctx.fillText('GEN-AI SPECIALIST', 256, 690);

    // Barcode fake
    ctx.fillStyle = '#ffffff';
    for(let i=0; i<20; i++) {
        const w = Math.random() * 10 + 2;
        ctx.fillRect(100 + (i*16), 800, w, 60);
    }
    ctx.font = '24px monospace';
    ctx.fillText('ID: 2024-VSDVLP', 256, 900);
